...
* **Comparison**: SpiceDB (Alternative 2) adds operational complexity (separate service, separate database, network dependency on every request) without providing Cedar's condition/ABAC capabilities — SpiceDB's relationship model would require workarounds for attribute-based conditions like "allow access only to clusters with label env=staging". A custom RBAC engine (Alternative 3) lacks a formal policy language, making future ABAC support a retrofit rather than a natural extension.
...
* Cross-namespace list authorization is handled at the database level (`WHERE namespace IN UNNEST(...)`) — no over-fetching or post-filtering
...
* **Observability**: Authorization decisions (allow/deny) are logged with principal, action, resource, and determining policy. Cedar evaluation latency is trackable via existing request metrics. Cache hit/miss rates are observable.
...
* Custom role namespace-pinning (Phase 6): user-defined roles are always scoped to their namespace via Cedar AST inspection — cross-namespace escalation is rejected at creation time
...
* Cross-namespace list queries: database-level filtering via `WHERE namespace IN UNNEST(...)` — no post-query filtering, pagination works natively
