...
We will use [Cedar](https://www.cedarpolicy.com/) as the authorization engine for Gecko's public (customer-facing) API. Cedar policies are generated from database-stored roles and evaluated in-process per request — no external authorization service on the request path. Roles and role bindings are stored in the same Spanner database as other Gecko resources. System roles are seeded by a role-seeder controller that reconciles a ConfigMap into the database via the private API (kube-apiserver, authorized by kube RBAC). User-defined roles are managed via the public API. Each regional Gecko instance is independently authoritative for its own roles and bindings.
...
- **Assumptions**: User identity is a Google account email extracted from the ESPv2-injected JWT claims header. System role definitions change infrequently (seeded by a controller on deployment). User-defined roles and role bindings change occasionally (admin operations). Authorization decisions are dominated by reads (every API request), not writes.

## Alternatives Considered

1. **Cedar (in-process policy evaluation)**: Embed the `cedar-go` library in `platform-api-server`. Generate Cedar policies from database-stored roles (seeded by an external controller via the private API). Evaluate policies per request against a cached entity graph built from RoleBinding/PlatformRoleBinding records in the database.
...
* **Evidence**: The proof-of-concept implementation (gecko `authz` branch) demonstrates the full authorization flow: role seeding via the private API, Cedar policy generation from database-stored roles, entity graph construction from database-backed stores, per-request evaluation, cross-namespace list filtering, and bootstrap. The implementation adds ~2,000 lines of Go with 59 tests and 127 subtests covering all role-action combinations.
...
* Cedar conditions enable attribute-based access control (user-defined roles with `when` clauses referencing resource attributes like region, labels)
...
* System role definitions are versioned in Git as a ConfigMap, reviewed via PR, and reconciled into the database by a role-seeder controller on each deployment. User-defined roles are managed via the public API by principals with `role.*` permissions.
...
* System role changes still require a deployment (the role-seeder controller reconciles from the ConfigMap). User-defined roles are modified via the public API without redeployment.
...
* Custom role namespace-pinning: user-defined roles are always scoped to their namespace via Cedar AST inspection — cross-namespace escalation is rejected at creation time. Generated policies constrain both the principal (`principal in Namespace::"<ns>"`) and the resource (`resource in Namespace::"<ns>"`), ensuring a principal bound in namespace A cannot access resources in namespace B even if a Cedar condition on resource attributes would otherwise match.
* Role validator infrastructure allow-list: user-defined roles may only include read-only infrastructure permissions (`cluster.list`, `cluster.get`, `nodepool.list`, `nodepool.get`); write/delete infrastructure permissions are rejected at creation time, preventing service-admins from creating `cluster-admin`-equivalent roles
...
* Entity cache: populated on first access per user, served from memory on subsequent requests, invalidated on binding writes for the affected user and on role mutations (broader invalidation of all users bound to the changed role)
...
* System roles are reconciled from a ConfigMap by the role-seeder controller via the private API (kube RBAC). The same controller creates the bootstrap platform-admin binding. No bootstrap code runs inside `platform-api-server`.
