...
- **Constraints**: The repository must support both the `orlop` framework and the `platform-api` application types.
...
* **Comparison**: Co-locating in `gcp-hcp` (alternative 2) would impose documentation-oriented CI/CD on a Go server project and make independent releases impractical.
...
* Dedicated repository provides clear separation of concerns for the Platform API
...
* Developers must navigate multiple repositories when working across API definition (gecko) and infrastructure (gcp-hcp-infra)
