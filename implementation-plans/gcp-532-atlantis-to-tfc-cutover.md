...
**Status (2026-08-21)**: Stories 1–5 are complete for integration — `terraform/atlantis-integration.yaml` no longer lists `global`, `region`, or `management-cluster` projects, all three run on TFC with `auto_apply = true` (validated in [GCP-951](https://redhat.atlassian.net/browse/GCP-951), now Closed). Region/MC onboarding automation (Story 6 script work) shipped under [GCP-535](https://redhat.atlassian.net/browse/GCP-535). `hypershift-ci` and `platform-ci` migrated to `gcp-hcp-ci` on TFC via [GCP-1093](https://redhat.atlassian.net/browse/GCP-1093) (complete 2026-08-21). 2 Atlantis projects remain unmigrated — `service`, `pagerduty` — neither was in the epic's original scope; see [Remaining Atlantis Projects](#remaining-atlantis-projects-not-yet-migrated) for target TFC projects and tracking tickets ([GCP-1092](https://redhat.atlassian.net/browse/GCP-1092), [GCP-1094](https://redhat.atlassian.net/browse/GCP-1094)). Also remaining: Story 6 doc finalization ([GCP-952](https://redhat.atlassian.net/browse/GCP-952)), Story 7/8 stage and production rollout ([GCP-953](https://redhat.atlassian.net/browse/GCP-953)), Story 9 decommission ([GCP-954](https://redhat.atlassian.net/browse/GCP-954)).
...
`terraform/atlantis-integration.yaml` still lists 2 projects beyond global/region/MC (`service`, `pagerduty`). None of the post-epic Atlantis projects were in the epic's original scope — `hypershift-ci` was listed but explicitly deferred ("planned separately after environment workspaces are validated," a milestone completed via GCP-951) and has since migrated (see [Migrated](#migrated) below); `platform-ci`, `service`, and `pagerduty` were added to Atlantis after the epic was written and were never scoped at all. The two remaining have been ticketed with a target TFC project:
