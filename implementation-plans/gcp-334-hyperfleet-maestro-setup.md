...
|---|---|---|---|
| `maestro-cloud-resources` | -5 | Pub/Sub topics, server subscriptions, GCP SAs + IAM for Maestro; includes consumer-registration CronJob that auto-registers MCs as Maestro consumers | custom chart in `gcp-hcp-infra` |
...
| Application | Wave | Purpose | Chart |
|---|---|---|---|
| `maestro-server` | 0 | Maestro messaging server (connects to region Pub/Sub) | upstream `openshift-online/maestro`, path `charts/maestro-server/` |
...
Chart paths above are verified against local `maestro` and `hyperfleet-sentinel` repos; for `hyperfleet-api`, path `charts/` is as used by upstream (e.g. in `hyperfleet-chart` / `hyperfleet-infra` dependencies).
...
| Maestro Agent GCP SA + publisher IAM on shared topics | `maestro-cloud-resources` chart (Config Connector) | Region cluster deploy |
| Per-MC subscriptions (`sourceevents-{mc}`, `sourcebroadcast-{mc}`) | Terraform (`management-cluster/maestro.tf`) | MC provisioned |
| Per-MC subscriber IAM on those subscriptions | Terraform (`management-cluster/maestro.tf`) | MC provisioned |
| WIF binding (MC k8s SA → Agent GCP SA) | Terraform (`management-cluster/maestro.tf`) | MC provisioned |
...
capacity-driven auto-scaling, it should move into a dedicated `maestro-consumer-registration`
