...
This repository uses the Jira plugin from [openshift-eng/ai-helpers](https://github.com/openshift-eng/ai-helpers).

## GCP HCP Team Skill

The `jira:gcp-hcp` skill provides team-specific conventions for creating issues in the GCP project (Hypershift on GKE):

- **Project Key**: GCP
- **Custom Fields**: Epic Name, Epic Link, Parent Link
- **Templates**: Story, Epic, and Feature templates in `docs/`
- **Definition of Done**: Completion criteria in `docs/definition-of-done.md`

The skill is automatically invoked when:
- Creating issues in the GCP project
...
**Keeping the index in sync:** When adding a new design decision, also add an entry to `design-decisions/INDEX.md` under the appropriate category.
