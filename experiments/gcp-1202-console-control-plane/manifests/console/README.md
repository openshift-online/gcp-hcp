...
| Route (`console`, `downloads`) | present | `$patch: delete` | CPO owns exposure Routes (see "Route ownership" above). It creates them passthrough, with the host derived from the APIServer host, because the router is SNI-passthrough only and the bridge terminates its own TLS. |
