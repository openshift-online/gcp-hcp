...
| `redhat-domain-admins.yaml` | ClusterRoleBinding granting cluster-admin to Google OIDC identities (redhat.com Workspace domain group + specific consumer account). Required because an externally-OIDC-authenticated guest cluster has no default RBAC for those identities. |
