...
| openshift/hypershift | Honour the `hypershift.openshift.io/hosted-cluster-sourced` day-2 secret annotation beyond ARO-HCP. Today gated by `azureutil.IsAroHCPByHCP` in `hostedclusterconfigoperator/controllers/resources/resources.go`; unchanged on current upstream `main`. | Not yet filed | Not yet filed | Design agreed | Needed for the day-2 console OIDC client model. Pitch as wanting the **guest-sourced** property, not the **isolation** property — the annotation's doc comment says such secrets "can't live on the control-plane", but a control-plane-side console must mount it. May warrant a distinct mode rather than reusing the ARO annotation. See [open-questions.md](open-questions.md) §1. |
| openshift/hypershift | Stop HCCO's guest `Authentication` mirror from being rejected when the console runs control-plane-side. `support/globalconfig/authentication.go` blindly copies the whole spec; the console `oidcClients` entry is rejected by CEL while nothing writes guest `status.oidcClients`, leaving `ConfigOperatorReconciliationSucceeded` False forever. | Not yet filed | Not yet filed | Design agreed | Two options: the ported console-operator writes the guest status, or prune console `oidcClients` from the guest mirror under control-plane placement. Not a wedge — HCCO aggregates errors and all other guest config still reconciles — but a permanently-False condition is unacceptable for a managed service. |
| openshift/console-operator | Sync the console OIDC client secret from guest `openshift-config` into the HCP namespace so the control-plane-side bridge can mount it. No such guest → management sync exists; the current flow runs the other way. | Not yet filed | Not yet filed | Design agreed | Part of the dual-kube-API port, see [operator-migration.md](operator-migration.md). |
| openshift/console | SNI-based serving-certificate selection in the bridge, so one pod can serve `console.<domain>` under the platform wildcard and a custom domain under a customer certificate. `GetCertificate` already has the right shape but discards the `*tls.ClientHelloInfo` (`cmd/bridge/main.go:816-826`). | Not yet filed | Not yet filed | Speculative — custom domain | **Untested.** Only if custom console domains are in scope, and avoidable even then by running a second console Deployment per custom domain. See [open-questions.md](open-questions.md) §3. |
| openshift/console-operator | Carry the custom console hostname and its TLS Secret from guest `Ingress.spec.componentRoutes[]` / `openshift-config` into the HCP namespace, and derive the extra Route there. | Not yet filed | Not yet filed | Speculative — custom domain | **Untested.** Same guest → management direction as the OIDC client secret row above; worth building once if both land. See [open-questions.md](open-questions.md) §3. |
