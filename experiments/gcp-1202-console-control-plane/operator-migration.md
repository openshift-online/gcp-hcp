...
- **Cross-cluster secret/config hand-off** — oauth-config, console-config CAs, `service-ca.crt` computed from guest data but mounted on MC bridge. library-go's `resourcesynccontroller` is single-cluster, so this needs a hand-rolled cross-cluster copy or direct fetch. Mostly **deferrable for core console + plugins** (the OIDC path does not need oauth-serving-cert; service-ca is guest-side only for plugin backends) — **with one exception that is not deferrable**, below.
- **The OIDC client secret, guest → management. Not deferrable.** Under the day-2 client model ([open-questions.md](open-questions.md) §1) the customer creates the console client secret in the guest's `openshift-config`, and the bridge — running on the management cluster — must mount it. No guest → management sync exists today; the existing HCCO flow runs the opposite way, HCP namespace → guest. This is the one piece of cross-cluster hand-off that the OIDC path *does* require, and it is a natural fit for the dual-client operator: read with the guest client, write with the management client.

  Two consequences worth designing for. First, this is [Cesar Wong's blast-radius concern](#blast-radius-concern-raised-in-review) made concrete — a customer-deletable guest Secret gates a control-plane workload, so deletion and rotation behaviour must be explicit rather than emergent. Second, it inverts the stated rationale of HyperShift's `hosted-cluster-sourced` annotation, which exists to keep such secrets *off* the control plane; our reason for wanting it is customer access, not secrecy. That distinction needs to be made plainly in the upstream ask.
